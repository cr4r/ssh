...
cd socks
patch="https://github.com/cr4r1/ssh/raw/master/install/backsocz"
arq="backsocz"
...
cd $HOME
msg="$2"
[[ $msg = "" ]] && msg="Selamat Datang"
portxz="$1"
[[ $portxz = "" ]] && portxz="8080"
screen -dmS sokz scktcheck "$portxz" "$msg" > /dev/null 2>&1
}
gettunel_fun () {
msg -ne " $(fun_trans "Masukkan Sandi untuk Gettunel"): " && read portt
echo "master=$prott" > ${SCPinst}/pwd.pwd
while read service; do
[[ -z $service ]] && break
echo "127.0.0.1:$(echo $service|cut -d' ' -f2)=$(echo $service|cut -d' ' -f1)" >> /etc/gettunel.pwd
done <<< "$(mportas)"
screen -dmS getpy python ${SCPinst}/PGet.py -b "0.0.0.0:$1" -p "/etc/gettunel.pwd"
 [[ "$(ps x | grep "PGet.py" | grep -v "grep" | awk -F "pts" '{print $1}')" ]] && {
 echo -e "$(fun_trans "Gettunel Berhasil Memulai")"
 msg -bar
 echo -ne "$(fun_trans "Sandi Getunnel anda"):"
 echo -e "\033[1;32m $portt"
 } || {
msg -bar
msg -ama "$(fun_trans "Gettunel belum dimulai")"
msg -bar
 }
}
pid_kill () {
...
remove_fun () {
msg -ama "$(fun_trans "Menghentikan") Socks Python"
msg -bar
...
echo -ne "\033[1;32m [7] > " && msg -azu "$(fun_trans "HENTIKAN SEMUA SOKET PYTHON")"
echo -ne "\033[1;32m [0] > " && msg -bra "$(fun_trans "Kembali")" && msg -bar
IP=(meu_ip)
while [[ -z $portproxy || $portproxy != @(0|[1-7]) ]]; do
msg -ne " $(fun_trans "Pilih")=> " && read portproxy
tput cuu1 && tput dl1
done
 case $portproxy in
    7)remove_fun && return;;
...
msg -ama " $(fun_trans "Pilih port mana yang akan didengarkan Soc_k")"
msg -bar
porta_socket=
while [[ -z $porta_socket || ! -z $(mportas|grep -w $porta_socket) ]]; do
msg -ne " $(fun_trans "Masukkan Port"): " && read porta_socket
tput cuu1 && tput dl1
done
msg -ama " $(fun_trans "Pilih Teks Koneksi")"
msg -bar
msg -ne " $(fun_trans "Masukkan pesan untuk Status"): " && read texto_soket
    case $portproxy in
