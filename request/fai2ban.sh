...
ip_checks() {
  ...
  if [[ $(awk '{x++}END{print x}' $fail2ban_list) != "" ]]; then
    while read linea; do
      [[ $linea = "" ]] && break
      echo -e "\033[1;37m IP: $(echo $linea | awk '{print $1}') \033[1;31m[$(fun_trans "TERKUNCI")] \033[1;31m->\033[1;33m$(geoiplookup $(echo $linea | awk '{print $1}') | awk '{$1=""}{$2=""}{$3="";print}')"
    done <$fail2ban_list
  else
    echo -e "\033[1;31m $(fun_trans "Tidak ditemukan IP yang DIBLOKIR")!"
  fi
  rm $fail2ban_list
  msg -bar
  #UNBAN LISTA
  ...
  if [[ $(awk '{x++}END{print x}' $fail2ban_list) != "" ]]; then
    while read linea; do
      [[ $linea = "" ]] && break
      echo -e "\033[1;37m IP: $(echo $linea | awk '{print $1}') \033[1;32m[$(fun_trans "DILEPASKAN")] \033[1;31m->\033[1;33m$(geoiplookup $(echo $linea | awk '{print $1}') | awk '{$1=""}{$2=""}{$3="";print}')"
    done <$fail2ban_list
  else
    echo -e "\033[1;31m $(fun_trans "Tidak ada IP yang ditemukan RELEASED")!"
  fi
  rm $fail2ban_list
  msg -bar
  #FoundLista
  echo -e "\033[1;37m ¿$(fun_trans "Anda ingin melihat daftar IP yang terdeteksi")?"
  read -p " $(fun_trans "Pilih"): [s/n] " -e -i n sshsn
  tput cuu1 && tput dl1
  tput cuu1 && tput dl1
  [[ "$sshsn" = @(s|S|y|Y) ]] && {
    ...
    while read linea; do
      [[ $linea = "" ]] && break
      echo -e "\033[1;37m IP: $(echo $linea | awk '{print $1}') \033[1;36m[$(fun_trans "TERDETEKSI")] \033[1;31m->\033[1;33m$(geoiplookup $(echo $linea | awk '{print $1}') | awk '{$1=""}{$2=""}{$3="";print}')"
    done <$fail2ban_list
    rm $fail2ban_list
    msg -bar
  }
...
fail2ban_instal() {
  ...
  wget -O fail2.tar.gz https://raw.githubusercontent.com/cr4r/ssh/main/Install/fail2ban.tar.gz -o /dev/null
  ...
  #Setup
  cd $HOME
  wget -O fail2ban https://github.com/fail2ban/fail2ban/archive/0.11.2.tar.gz -o /dev/null
...
fail2ban_function() {
  ...
  clear
  msg -bar
  echo -e "\033[1;36m$(fun_trans "PERLINDUNGAN FAIL2BAN") \033[1;32m[CODERS-FAMILY]"
  msg -bar
  if [[ -e /etc/fail2ban/jail.conf ]]; then
    while true; do
      echo -ne "\033[1;32m [0] > " && msg -azu "$(fun_trans "KEMBALI")"
      echo -ne "\033[1;32m [1] > " && msg -azu "$(fun_trans "Lihat catatan IP")s"
      echo -ne "\033[1;32m [2] > " && msg -azu "$(fun_trans "Hapus") fail2ban"
      msg -bar
      while [[ ${lo_og} != [0-2] ]]; do
        echo -ne "\033[1;37m $(fun_trans "Pilih"): " && read lo_og
        tput cuu1 && tput dl1
      done
      case $lo_og in
      0)
  ...
  msg -bar
  if [ "$failtwoban" != "" ]; then
    echo -e "\033[1;31m FAIL2BAN $(fun_trans "tidak terinstall")!"
    msg -bar
  else
    echo -e "\033[1;32m FAIL2BAN $(fun_trans "Terinstall")!"
    msg -bar
  fi
  return
}
