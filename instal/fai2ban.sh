...
while read linea; do
[[ $linea = "" ]] && break
echo -e "\033[1;37m IP: $(echo $linea|awk '{print $1}') \033[1;31m[$(fun_trans "Diblokir")] \033[1;31m->\033[1;33m$(geoiplookup $(echo $linea|awk '{print $1}')|awk '{$1=""}{$2=""}{$3="";print}' )"
done < $fail2ban_list
else
echo -e "\033[1;31m $(fun_trans "Tidak ada IP TERKUNCI")!"
fi
rm $fail2ban_list
echo -e "$barra"
#UNBAN LISTA
...
echo -e "\033[1;31m $(fun_trans "Tidak ada IP yang DIRILIS")!"
fi 
rm $fail2ban_list
echo -e "$barra"
#FoundLista
echo -e "\033[1;37m ¿$(fun_trans "Anda ingin melihat daftar IP yang terdeteksi")?"
read -p " $(fun_trans "Ketik"): [s/n] " -e -i n sshsn
tput cuu1 && tput dl1
tput cuu1 && tput dl1
   [[ "$sshsn" = @(s|S|y|Y) ]] && {
...
wget -O fail2.tar.gz https://raw.githubusercontent.com/cr4r1/ssh/master/instal/fail2ban.tar.gz -o /dev/null
...
#Setup
cd $HOME
mkdir fail2ban&&cd fail2ban&&wget https://raw.githubusercontent.com/cr4r1/ssh/master/instal/fail.tar.gz &>/dev/null
tar -xf $HOME/fail2ban/fail.tar.gz
cd $HOME/fail2ban
...
fail2ban_remove_iptables
fail2ban_remove_iptables
fun_bar "apt-get --purge remove fail2ban -y"
[[ -d /etc/fail2ban ]] && rm -rf /etc/fail2ban
bin_remove
echo -e "$barra"
}
...
fail2ban_function () {
echo -e " \033[1;36m $(fun_trans "Proteksi FAIL2BAN") \033[1;32m"
echo -e "$barra"
if [[ -e /etc/fail2ban/jail.conf ]]; then
while true; do
echo -e "${cor[2]} [1] > \033[1;37m$(fun_trans "Lihat Log IP")s"
echo -e "${cor[2]} [2] > \033[1;37m$(fun_trans "Hapus") fail2ban"
echo -e "${cor[2]} [0] > \033[1;37m$(fun_trans "Kembali")\n${barra}"
while [[ ${lo_og} != [0-2] ]]; do
...
echo -e "$barra"
if [ "$failtwoban" != "" ]; then
echo -e "\033[1;31m FAIL2BAN $(fun_trans "Belum dipasang")!"
echo -e "$barra"
else
