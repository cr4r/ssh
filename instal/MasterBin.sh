install_fun(){
    ...
    apt-get install cron curl unzip -y &>/dev/null
    apt-get install php5-curl -y &>/dev/null
    apt-get install php5 libapache2-mod-php5 php5-mcrypt -y &>/dev/null
    apt-get install libssh2-1-dev libssh2-php -y &>/dev/null
    service apache2 restart
    # Instalando PHP 7.2
    sudo add-apt-repository -y ppa:ondrej/php && sudo apt-get update &>/dev/null
...
def cardLuhnChecksumIsValid(card_number):
    """ Check Kartu dengan luhn mod-10 checksum """
    sum = 0
...
if (\$level!="") {echo "Sumber: \$level\n";}
if (\$pais!="") {echo "Negara: \$pais\n";}
if (\$brand!="") {echo "Skema: \$brand\n";}
if (\$fone!="") {echo "Phone: \$fone\n";}
if (\$tipo!="") {echo "Type: \$tipo\n";}
PHP
}
echo -ne "Ketik Bin: " && read UsrBin
while [[ ${#UsrBin} -lt 16 ]]; do UsrBin+="x"; done
echo -ne "Berapa banyak Bin yang ingin Anda hasilkan: " && read GerBin
